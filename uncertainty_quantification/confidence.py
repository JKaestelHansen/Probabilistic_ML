...
def quantile_and_oracle_errors(uncertainties, errors, number_quantiles):
    ...
    for q in qs:
        idx = (
            s.sort_values(by="unc", ascending=False).reset_index(drop=True)
            <= s.quantile(q)
        ).values[:, 0]

        quantile_errs.append(
            np.mean(
                s.sort_values(by="unc", ascending=False)
                .reset_index(drop=True)[idx]["err"]
                .values
            )
        )

        idx = (
            s.sort_values(by="err", ascending=False).reset_index(drop=True)
            <= s.quantile(q)
        ).values[:, 1]
        oracle_errs.append(
            np.mean(
                s.sort_values(by="err", ascending=False)
                .reset_index(drop=True)[idx]["err"]
                .values
            )
        )
...
def ranking_confidence_curve(losses: np.array, uncertainties: np.array, quantiles=10):
    """
    AUTHOR: Richard M, edited description Jacob KH
    ...
    average_loss_in_quantile: average loss (error) in each quantile of uncertainty i.e returns a
    list of average errors for each quantile of uncertainty measuring error as predictions
    with large unceratinty are gradually removed

    average_loss_in_oracle_quantile: average loss (error) in each quantile of oracle errors i.e returns a
    ...
    average_loss_in_quantile = np.array(
        [
            np.average([t[0] for t in sorted_tuples if t[1] <= h_q])
            for h_q in h_quantiles
        ]
    )
    average_loss_in_oracle_quantile = np.array(
        [np.average([t[0] for t in sorted_tuples if t[0] <= h_o]) for h_o in h_oracle]
    )
...
def area_confidence_oracle_error(uncertainties, oracle_error, quantiles=10):
    ...
    quartile_diffs = np.array(
        [unc - oracle for unc, oracle in zip(uncertainties, oracle_error)]
    )
...
def decreasing_ratio(h_quantiles: np.ndarray) -> float:
    ...
    larger_quantile_set = np.array(
        [
            int(h_i >= next_h_i)
            for h_i, next_h_i in zip(h_quantiles[:-1], h_quantiles[1:])
        ]
    )
